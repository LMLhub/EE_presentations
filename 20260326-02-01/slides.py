...
def app_slide_leverage() -> None:
    ...
    #Panel 1
    ax[0].plot(t, path, color=color_blue, label = f"Trajectory, l = {leverage}", alpha=1, lw=0.8)
    
    # Theoretical expectation
    # ax.plot(t, S0*np.exp((mu_r + leverage * mu_e)*t), color= color_red, lw=2, ls="-", label="Expectation value")

    # Theoretical time average
    ax[0].plot(t, S0*np.exp((mu_r + leverage * mu_e - leverage**2 * 0.5 * sigma_s**2) * t), color=color_green, lw=2, ls="-", label="Theoretical time average")

    # Theoretical maximum time average
    lev_opt = mu_e / sigma_s**2
    ax[0].plot(t, S0*np.exp((mu_r + lev_opt * mu_e - lev_opt**2 * 0.5 * sigma_s**2) * t), color=color_grey, lw=0.8, ls="--", label="Maximum time average")
